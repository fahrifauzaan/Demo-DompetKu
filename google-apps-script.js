...
// ===================== KONFIGURASI =====================
const VALID_SHEETS = ['Transactions', 'Accounts', 'Fixed Income Investment', 'FixedIncome', 'BudgetCategories', 'Debts', 'Settings', 'AssetsNonLiquid', 'Saham', 'Crypto', 'Reksadana', 'Goals', 'Recurring', 'Insurance', 'MonthlyBudgets', 'Retirement'];
...
// 'FixedIncome' = tab DATA BIASA (plain) untuk SBN/Deposito/P2P, menggantikan dashboard
// 'Fixed Income Investment' yang ber-merged-cell (tak kompatibel jalur API). Tab lama tetap dibaca.
const AUTO_CREATE_SHEETS = ['Goals', 'Recurring', 'Insurance', 'MonthlyBudgets', 'Retirement', 'FixedIncome'];
...
function updateInSheet(sheetName, id, data) {
  ...
  // Fallback: aset Fixed Income lama mungkin masih tersimpan di tab dashboard 'Fixed Income Investment'.
  if (sheetName === 'FixedIncome') return updateFixedIncome(id, data);
  return false;
}
...
function deleteFromSheet(sheetName, id) {
  ...
  // Fallback: aset Fixed Income lama mungkin masih di tab dashboard 'Fixed Income Investment'.
  if (sheetName === 'FixedIncome') return deleteFixedIncome(id);
  return false;
}
